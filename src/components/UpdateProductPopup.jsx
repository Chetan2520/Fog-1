...
import { X, Package, DollarSign, Percent, Sparkles } from "lucide-react";
...
const UpdateProductPopup = ({ 
  ...
  const [formData, setFormData] = useState({
    ...
    newPrice: selectedProduct?.newPrice || selectedProduct?.price || '',
  ...
  const handleSubmit = async () => {
    try {
      ...
      setProducts(products.map(p => (p._id === selectedProduct._id || p.id === selectedProduct.id) ? result : p));
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl animate-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          ...
          <button
            onClick={() => { setShowUpdatePopup(false); setSelectedProduct(null); }}
